...
parser.add_argument('--lrepochs', default="192,256:2", type=str, help='the epochs to decay lr: the downscale rate')
# parser.add_argument('--lrepochs',default="300,500:2", type=str,  help='the epochs to decay lr: the downscale rate')
parser.add_argument('--logdir', default='./file_ckpt/kitti2012_noc/',
                    help='the directory to save logs and checkpoints')
parser.add_argument('--loadckpt', default='./pretrained_model/kitti2015.ckpt',
...
def train():
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        if (epoch_idx + 1) % args.save_freq == 0:
            ...
            torch.save(checkpoint_data, "{}/{}.checkpoint_{:0>6}.ckpt".format(args.logdir, 'KITTI', epoch_idx))
        gc.collect()
        ...
        if (epoch_idx) % 1 == 0:
            ...
            avg_test_scalars_df = pd.DataFrame(avg_test_scalars)
            avg_test_scalars_df.to_csv('{}{}.{}.{}.csv'.format(args.logdir, epoch_idx, args.epochs, 'kitti2015'))
            gc.collect()
