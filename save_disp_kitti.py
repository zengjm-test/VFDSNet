...
parser.add_argument('--testlist', default='./filenames/kitti2015_test.txt', help='testing list')
...
save_dir = './file_ckpt/VFDSNet/kitti2015/'
...
def test():
    ...
    for batch_idx, sample in enumerate(TestImgLoader):
        ...
        for disp_est, fn in zip(disp_est_np, left_filenames):
            ...
            disp_est = np.array(disp_est[8:, :-7], dtype=np.float32)
            fn = os.path.join(save_dir, fn.split('/')[-1])
            print("saving to", fn, disp_est.shape)
            # disp_est = np.squeeze(disp_est)
